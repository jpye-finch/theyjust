...
**Deviation recorded during implementation:** `assets/images/notification-icon.png` does not exist and was not created — everything in `assets/images/` at implementation time was still Expo scaffold artwork (expo-logo, react-logo, an `android-icon-monochrome.png` that is Expo's own mark), and using any of it would have put Expo's logo in a parent's status bar. The plugin was registered with `color` only; the `icon` key was omitted entirely rather than pointed at a placeholder. Android falls back to its default notification icon — wrong-looking, but honest, and not fixable without a real design pass. **A monochrome 96×96 white-on-transparent notification icon is still outstanding** and should replace this before release; once it exists, add the `icon` key back pointing at it.

**Files:**
- Modify: `app.json`
- ~~Create: `assets/images/notification-icon.png`~~ — not created; see deviation note above.
